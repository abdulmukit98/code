...
ll processing(ll arr[], int upper, int lower);
...
int main() {
    ...
    ll min = processing(stone, upper, lower);
...
ll processing(ll arr[], int upper, int lower)
{
    if(upper < lower) return LONG_LONG_MAX; // ignore base return
    ...
    return min(abs(team_zero - team_one), processing(arr, upper-1, lower));
